...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var pizzaContainers = Array.prototype.slice.call(document.querySelectorAll(".randomPizzaContainer"));
    var newWidths = [];
    ...
    for (var i = 0; i < pizzaContainers.length; i++) {
      pizzaContainers[i].style.width = newWidths[i];
    }
  }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(subphase + (i % 5));
    //items[i].style.transform = 'translateX('+(items[i].basicLeft + 100 * phase) + 'px) translateZ(0)';
